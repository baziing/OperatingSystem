...
char WHITE_FLAG='O';
char BLACK_FLAG='@';
char map[WIDTH][HIGHT];//棋盘 
int statePC[WIDTH][HIGHT]={0};
int statePlayer[WIDTH][HIGHT]={0};
int player = BLACK; 
...
void draw_map();
int horizon(int hang, int lie, char whoFlag);
int vertic(int hang, int lie, char whoFlag);
int left(int hang, int lie, char whoFlag);
int right(int hang, int lie, char whoFlag);
int jieguo( int left, int right, int count, int k, char num );
void pcLoad();
void score();
int playerLoad();
...
void pcLoad(){
    ...
    for( i=0; i< 15; i++ ){
        for( j=0; j< 15; j++ ){
            if( statePC[i][j] > count ){
                count = statePC[i][j];
                hang = i;
                lie = j;
            }
            if( statePlayer[i][j] > count ){
                count = statePlayer[i][j];
                hang = i;
                lie = j;
            }
        }
    ...
    printf("statePC[%d][%d]:%d\n",hang,lie,statePC[hang][lie]);
    printf("statePlayer[%d][%d]:%d\n",hang,lie,statePlayer[hang][lie]);
...
void score(){
    ...
    for( n=0;n<15;n++){
        for( m=0;m<15; m++ ){
            statePC[n][m] = horizon(n,m,BLACK_FLAG)+vertic(n,m,BLACK_FLAG)+left(n,m,BLACK_FLAG)+right(n,m,BLACK_FLAG);
            statePlayer[n][m] = horizon(n,m,WHITE_FLAG)+vertic(n,m,WHITE_FLAG)+left(n,m,WHITE_FLAG)+right(n,m,WHITE_FLAG);
        }
    }   
}
//玩家下子
