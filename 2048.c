...
void getColor(uint16_t value, char *color, size_t length) {
	...
	if (value > 0) while (value >>= 1)// value不断右移一位，直到值变为0，实现每个二进制一个不同的颜色
	{
...
//旋转数据板，向右旋转90度，用一个方向的数组移动间接控制四个方向的移动
...
int main(int argc, char *argv[]) {
	...
	memset(board,0,sizeof(board));// 添加两次随机数,因为初始化时产生2个随机数
	addRandom(board);addRandom(board);// 绘制数据板
	drawBoard(board);// 禁用缓存输入，终端支持按字符读取且不回显
	setBufferedInput(false);// 游戏主循环
	while (true) {
		c=getchar();
		switch(c) {
			case 68:	// left arrow
